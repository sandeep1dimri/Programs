...
CSV_PATH = '.'
RENAME_CSV_FILE_NAME = 'File_rename.csv'
...
root = tk.Tk()
root.title("Select Source Folder")
root.withdraw()
SOURCE_FOLDER = filedialog.askdirectory()

root = tk.Tk()
root.title("Select Target Folder")
root.withdraw()
TARGET_FOLDER = filedialog.askdirectory()

root = tk.Tk()
root.title("Select rename instruction file")
root.withdraw()
file_with_abs_path = filedialog.askopenfilename()
#root.mainloop()
#root.withdraw()
'''
instruction file format:
,OldImageName,NewImageName
0,image_0085 copy.jpg,image_0085.jpg
'''
...
if __name__ == "__main__":
    ...
    for index, row in df_rename.iterrows():
        print("Processing file:", row['OldImageName'], 'NewImageName:',row['NewImageName'] ,end="")
        ...
        if df_temp.shape[0] > 1:
            ...
            if not row['NewImageName'].strip(): 
                path_with_spaces =  TARGET_FOLDER+os.sep + row['OldImageName'].strip().replace(' ', '')
            else:
                path_with_spaces =  TARGET_FOLDER+os.sep + row['NewImageName'].strip().replace(' ', '')
